...
namespace DapperExtensions
{
    public interface IDapperImplementor
    {
        ...
        //创建临时表
        bool CreateTempTable<T>(IDbConnection connection, T entity, string tempdbname, short dbRowSn) where T : class;
    }
    ...
    public class DapperImplementor : IDapperImplementor
    {
        ...
        /// <returns></returns>
        public bool CreateTempTable<T>(IDbConnection connection, T entity, string tempdbname, short DbRowSn) where T : class
        {
            var classMap = SqlGenerator.Configuration.GetMap<T>();
            var sql = SqlGenerator.CreateTempTable(classMap, tempdbname, DbRowSn);
            return connection.Execute(sql, null, null, null, CommandType.Text) > 0;
        }
