...
namespace DapperExtensions.Sql
{
    public interface ISqlGenerator
    {
        ...
        /// <param name="dbRowSn"></param>
        /// <returns></returns>
        string CreateTempTable(IClassMapper classMap, string tempdbname, short dbRowSn);
    }
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        public virtual string GetColumnName(IClassMapper map, IPropertyMap property, bool includeAlias)
        {
            ...
            return Configuration.Dialect.GetColumnName(string.Empty, property.ColumnName, alias);
        }
        ...
        /// <param name="dbRowSn"></param>
        /// <returns></returns>
        private string GetTableScheme(IClassMapper classMap, IEnumerable<IPropertyMap> columns, short dbRowSn)
        {
            var sbBuilder = new StringBuilder();
            if (dbRowSn == 1)
            {
                sbBuilder.Append("Sn INT IDENTITY(1,1) NOT NULL,");
            }
            foreach (var column in columns)
            {
                sbBuilder.Append(GetColumnName(classMap, column, false));
                sbBuilder.Append(" ");

                var sqltypestring = SqlTypeConvertHelper.GetSqlType(column.PropertyInfo.PropertyType);
                if (string.IsNullOrEmpty(sqltypestring))
                    throw new Exception("c#类型转换为Tsql类型失败！");
                sbBuilder.Append(sqltypestring);
                sbBuilder.Append(",");
            }
            var builder = sbBuilder.ToString();
        ...
        /// <param name="dbRowSn"></param>
        /// <returns></returns>
        public string CreateTempTable(IClassMapper classMap, string tempdbname, short dbRowSn)
        {
            ...
            // var parameters = columns.Select(p => Configuration.Dialect.ParameterPrefix + p.Name);
            string sql = $"CREATE TABLE {tempdbname} ({GetTableScheme(classMap, propertyMaps, dbRowSn)});";
            return sql;
        }
