...
namespace DapperExtensions.Sql
{
    public interface ISqlGenerator
    {
        ...
        string CreateTable(IClassMapper classMap, string tempdbname, short DbRowSn);
    }
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        /// <returns></returns>
        public virtual string CreateTable(IClassMapper classMap, string tempdbname, short DbRowSn)
        {
            var columns = classMap.Properties.Where(p => !(p.Ignored || p.IsReadOnly || p.KeyType == KeyType.Identity));
            if (!columns.Any())
            {
            ...
            string sql = string.Format("CREATE TABLE {0} ({1});",
             tempdbname, GetTableScheme(classMap, columns, DbRowSn));
            return sql;
        }
