...
namespace DapperExtensions
{
    public interface IDapperExtensionsConfiguration
    {
        ...
        IClassMapper GetMapIncludeForeignKey(Type entityType);
    ...
    public class DapperExtensionsConfiguration : IDapperExtensionsConfiguration
    {
        ...
        private readonly ConcurrentDictionary<Type, IClassMapper> _classMaps1 = new ConcurrentDictionary<Type, IClassMapper>();
        public DapperExtensionsConfiguration()
            : this(typeof(AutoClassMapper<>), new List<Assembly>(), new SqliteDialect())
        {
        }
        ...
        public IClassMapper GetMapIncludeForeignKey(Type entityType)
        {
            IClassMapper map;
            if (!_classMaps1.TryGetValue(entityType, out map))
            {
