...
namespace ZilLion.Core.Unities.ExtensionMethods.Reflection.CreateInstance
{
	...
	internal static class GetterSetterFactory
	{
		...
		{
			IGetValue property = (IGetValue)SGetterDict[propertyInfo];
			if( property == null ) {
				property = CreatePropertyGetterWrapper(propertyInfo);
				SGetterDict[propertyInfo] = property;
			}
			return property;
		}
		...
		{
			ISetValue property = (ISetValue)SSetterDict[propertyInfo];
			if( property == null ) {
				property = CreatePropertySetterWrapper(propertyInfo);
				SSetterDict[propertyInfo] = property;
			}
			return property;
		}
